...
class MessagesPanel(RecycleView):
    @staticmethod
    ...
    def get_message(self, message_id: int = None):
        ...
        for message_info in messages_info:
            ...
            date = message_info[-3]
            if not self.message_blocks.children:  # создание блока сообщения
                self.add_block(date)
            elif self.message_blocks.children[0].date != date:  # если блок есть, просто добавляю сообщение
                self.add_block(date)
    ...
    def add_block(self, date):
        ...
        new_block.size[1] = 37
        self.message_blocks.add_widget(new_block)
        self.message_blocks.size[1] += new_block.size[1] + self.message_blocks.spacing

    def show_message(self, **kwargs):
        """ В kwargs должны быть определены все параметры сообщения. """
        content_type = kwargs.get('content_type')
        if content_type == 'text':
            message = TextMessage(messages_panel=self, **kwargs)
            message.content_label.text = message.text
            message.time_label.text = message.time[:5]
        elif content_type == 'image':
            message = ImageMessage(messages_panel=self, **kwargs)
            message.image.source = message.image_path
        elif content_type == 'interactive':
            message = InteractiveMessage(messages_panel=self, **kwargs)
        ...
        message.block = self.message_blocks.children[0]
        Clock.schedule_once(message._rescale, 0)
        self.message_blocks.children[0].add_widget(message)
...
class Container(BoxLayout):
    ...
    def get_data_from_user(self):
        ...
        if self.text_input.text != '':
            self.messages_panel.handle_message('user', 'text', self.text_input.text)
            self.text_input.text = ''
            message = self.messages_panel.get_message(self.messages_panel)  # show last message
    ...
    def load_chat(self):
        ...
        self.messages_panel.get_message(message_id=True)  # show all messages
